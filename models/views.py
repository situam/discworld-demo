...
class ExpandedPersonListView:
    ...
    def link_to_prev_page(self):
        return None if self.person_list.previous is None else url_to_query_string(self.person_list.previous)
    
    @property
    def link_to_next_page(self):
        return None if self.person_list.next is None else url_to_query_string(self.person_list.next)
